...
public class FocusKeepRecyclerView extends RecyclerView {
    ...
    public void requestChildFocus(View child, View focused) {
        LogUtils.i(TAG, "requestChildFocus() nextchild= " + child + ",focused = " + focused);
        if (!hasFocus()) {
        ...
        mCurrentFocusPosition = getChildViewHolder(child).getLayoutPosition();
        LogUtils.i(TAG, "requestChildFocus() focusPos = " + mCurrentFocusPosition);
    }

    // 实现焦点记忆的关键代码
    @Override
    public void addFocusables(ArrayList<View> views, int direction, int focusableMode) {
        LogUtils.i(TAG, "addFocusables() direction =" + direction);
        View view = null;
        ...
        if (this.hasFocus() || mCurrentFocusPosition < 0 || (view = layoutManager.findViewByPosition(mCurrentFocusPosition)) == null) {
            super.addFocusables(views, direction, focusableMode);
    ...
    protected int getChildDrawingOrder(int childCount, int i) {
        ...
        LogUtils.i(TAG, "getChildDrawingOrder() focusedChild =" + focusedChild);
        if (focusedChild == null) {
            return super.getChildDrawingOrder(childCount, i);
        } else {
            int index = indexOfChild(focusedChild);
            LogUtils.i(TAG, "getChildDrawingOrder() index = " + index + ",i=" + i + ",count=" + childCount);
            if (i == childCount - 1) {
                return index;
            }
            if (i < index) {
