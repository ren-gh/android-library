...
public class CRCUtils {
    ...
    public static String genCRCStr(String rt, String oid, String im, String ts, String data) {
        String content = rt + oid + im + ts + data;
        return genCRC(content);
    }

    public static String genCRCStr(String content) {
        return genCRC(content);
    }

    private static String genCRC(String content) {
        int index = 0;
        int crcIndex = 0;
        int tableEntry = 0;
        int contentLength = content.length();
        for (index = 0x0; index < contentLength; index++) {
            int charHex = (int) content.charAt(index);
            int tempIdx = (crcIndex & 0xF) | (charHex & 0xF) << 0x4;
            tableEntry = crc_table[tempIdx];
            crcIndex = crcIndex >> 0x4 ^ tableEntry;
            tableEntry = crc_table[(crcIndex & 0xF) | (charHex & 0xF0)];
            crcIndex = crcIndex >> 0x4 ^ tableEntry;
        }
        String crc = Integer.toHexString(crcIndex);
        return crc;
    }
}
