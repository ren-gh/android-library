...
public class SortArray {
    ...
    public void selectSort(int[] array) {
        ...
        for (int i = 0; i < array.length - 1; i++) {
            ...
            if (minIndex != i) {
                BaseUtils.swap(array, i, minIndex);
                if (BaseUtils.DEBUG)
                    BaseUtils.print(array);
            }
        }
    ...
    public void quickSort(int[] array, int start, int end) {
        ...
        while (true) {
            ...
            if (i < j) {
                BaseUtils.swap(array, i, j);
            } else {
                break;
            }
        }
        // 交换 j和分界点的值
        BaseUtils.swap(array, start, j);
        if (BaseUtils.DEBUG)
            BaseUtils.print(array);
        // 递归左子序列
    ...
    private int partitionIt(int[] array, int start, int end, int pivot) {
        ...
        while (true) {
            ...
            if (leftPtr >= rightPtr)
                ...
                BaseUtils.swap(array, leftPtr, rightPtr);
        }
        BaseUtils.swap(array, leftPtr, end - 1);
        return leftPtr;
    }
    ...
    private int medianOf3(int[] array, int start, int end) {
        ...
        if (array[start] > array[center])
            BaseUtils.swap(array, start, center);

        if (array[start] > array[end])
            BaseUtils.swap(array, start, end);

        if (array[center] > array[end])
            BaseUtils.swap(array, center, end);

        BaseUtils.swap(array, center, end - 1);// put pivot on right
    ...
    private void manualSort(int[] array, int start, int end) {
        ...
        if (size == 3) {
            if (array[start] > array[end - 1])
                BaseUtils.swap(array, start, end - 1);
            if (array[start] > array[end])
                BaseUtils.swap(array, start, end);
            if (array[end - 1] > array[end])
                BaseUtils.swap(array, end - 1, end);
        }
    }
